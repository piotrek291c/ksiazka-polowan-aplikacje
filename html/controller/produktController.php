...
class produktController extends baseController {
    ...
    public function index() {
        ...
        $db = $this->registry->db;
        $results = $db::getRejonList();
        $produkty = array();
        foreach ($results as $row) {
            $produkt = new Produkt();
            $produkt->setidRejon($row['idRejon']);
            $produkt->setMiejscowosc($row['Miejscowosc']);
            $produkt->setObwod_lowiecki($row['Obwod_lowiecki']);
            $produkt->setNazwa_lowiska($row['Nazwa_lowiska']);                    
            $produkty[] = $produkt;
        }
        $kategorie = array();
        $results = $db::getRejonList();
